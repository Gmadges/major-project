...
int Server::run()
{
	auto func = [] (void * arg) {
		...
		// have to pass the context as a void* like some peasent
		//zmq::context_t *cont = (zmq::context_t *)_context;

		zmq::context_t *cont = (zmq::context_t *)arg;

		zmq::socket_t socket(*cont, ZMQ_REP);
	...
	{
		workers.push_back(std::thread(func, (void*)&context));
	}
